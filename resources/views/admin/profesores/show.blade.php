...
@section('title', 'Profesor ' . $profesore->user->name)
...
@section('content')
    <div class="card w-full md:w-1/2 m-auto ">
        <div class="card-header p-4 flex ">
            <a href="{{ route('admin.profesores.edit', $profesore) }}"
                class="btn-Modificar text-white text-center w-1/3">Editar
                Profesor</a>
            <form action="{{ route('admin.profesores.desactivar', $profesore) }}" method="POST">
                @csrf
                @method('put')
        ...
        <div class="card-body pl-4">
            <div class="my-2">
                <h1 class="text-gray-800 "><span class=" font-bold text-green-700 text-lg ">Nombre:
                    ...
                    {{ $profesore->user->name }} </h1>
            </div>
            <div class="my-2">
                <h1 class="text-gray-800 "><span class=" font-bold text-green-700 text-lg ">Primer apellido:
                    </span>
                    {{ $profesore->user->primer_ap }} </h1>
            </div>
            <div class="my-2">

                <h3 class="text-gray-800 "><span class="text-lg  font-bold text-green-700">Segundo apellido </span>
                    {{ $profesore->user->segundo_ap }}</h3>
            </div>
            <div class="my-2">
                <h3 class="text-gray-800 "><span class="text-lg  font-bold text-green-700">Especialidad : </span>
                    {{ $profesore->especialidad }}</h3>
            </div>
            <div class="my-2">
                <h3 class="text-gray-800 "><span class="text-lg  font-bold text-green-700">Descripcón Especialidad : </span>
                    {{ $profesore->descripcion_especialidad }}</h3>
            </div>
        </div>
    </div>
@stop
